...
6.  To deploy the newest version of Clean Slate, run `bash deploy.sh`. We deploy a new version of Clean Slate at least twice a week, on Monday and Tuesday. We recommend deploying every Sunday. However, before deploying, make sure to check the `CHANGELOG.md`. That is we will cover any breaking changes. 
