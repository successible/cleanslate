...
export const calculateFoodDensities = (food: Food): [number, number] => {
  const {
    caloriesPerCount,
    caloriesPerGram,
    countToGram,
  ...
  if (caloriesPerGram && proteinPerGram) {
    return [
      getCaloricDensityFromPerGram(caloriesPerGram),
      getProteinDensityFromPerGram(caloriesPerGram, proteinPerGram),
    ]
    ...
    return [getCaloricDensityFromPerGram(caloriesPerGram || 0), 0]
    ...
    return [
      getCaloricDensityFromCount(countToGram, caloriesPerCount),
      getProteinDensityFromCount(caloriesPerCount, proteinPerCount),
    ]
  } else {
    return [-1, -1]
  }
...
export const calculateRecipeDensities = (
  ...
  const proteinDensity = round(
    (proteinConsumed / caloriesConsumed) * 4 * 100,
    0
  )
  ...
  return [caloricDensity, proteinDensity]
}
