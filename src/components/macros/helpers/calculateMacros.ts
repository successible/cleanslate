...
import { QuickAddUnit, Unit } from '../../../models/Log/types'
...
const calculatePerFood = (
  ...
  if (unit === 'GRAM') {
    if (countToGram && (perCount || perCount === 0)) {
      ...
      return handleError(
        'Something went wrong with calculatePerFood and GRAM',
        { params: { data } }
      )
    }
    ...
    const grams = convertFromImperialToGrams(unit, amount)
    if (countToGram && (perCount || perCount === 0)) {
      ...
      return handleError(
        'Something went wrong with calculatePerFood and OZ/LBS',
        { params: { data } }
      )
    }
  } else {
    ...
    if ((countToGram || countToGram === 0) && (perGram || perGram === 0)) {
      ...
      return handleError(
        'Something went wrong with the calculatePerFood and COUNT',
        { params: { data } }
      )
    }
  }
...
export const calculatePerMacroPerRecipe = (
  ...
  const result = recipe.ingredients.reduce((acc, ingredient) => {
    ...
    if (food) {
      ...
      return handleError(
        'Something went wrong with calculatePerMacroPerRecipe',
        { params: { amount, metric, recipe } }
      )
    }
  }, 0)
