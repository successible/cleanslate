...
export const editor: StoreonModule<CleanslateSlices, EditorEvents> = (
  ...
  store.on('saveSearchResult', (state, event) => {
    return dotProp.set(state, 'editor.searchResult', event)
  })

  store.on('saveDummyFood', (state, event) => {
    return dotProp.set(state, 'editor.dummyFood', event)
  })
